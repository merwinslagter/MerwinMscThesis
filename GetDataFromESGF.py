...
# ESGF Metagrid. Adjust parameters based on target
...
proceed = input ("\nType 'ok' to continue: ").strip().lower()
if proceed != "ok":
    exit()
...
for source_ep in set(f[0] for f in files_to_transfer):
    ...
    print(f"Submitted transfer from {source_ep} with task ID:", transfer_result["task_id"])
